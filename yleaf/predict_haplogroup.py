...
def read_yleaf_out_file(
    ...
    haplotype_dict = {}
    parsed = []                    # (pos, marker, haplogroup, state) for non-~ rows
    pos_haplogroups = {}           # pos -> set of distinct haplogroups seen at that position
    with open(file) as f:
        f.readline()
        for line in f:
            cols = line.strip().split("\t")
            pos, marker, haplogroup, state = cols[1], cols[2], cols[3], cols[10]
            ...
            if haplogroup.endswith("~"):
                continue
            parsed.append((pos, marker, haplogroup, state))
            pos_haplogroups.setdefault(pos, set()).add(haplogroup)

    # Recurrent (homoplastic) positions map to >=2 distinct haplogroups (e.g. M145
    # defines both DE and, by independent recurrent mutation, R-BY109788). They stay
    # visible in the .out/.fmf but must NOT feed any QC score: a homoplasy in the
    # ancestral state on a lineage it does not define would otherwise penalise that
    # lineage's QC1/QC2/QC3. The same physical position is kept per-haplogroup in the
    # position files (Yleaf.py per-(haplogroup,pos) dedup), so both copies appear here.
    recurrent_positions = {p for p, hgs in pos_haplogroups.items() if len(hgs) >= 2}

    for pos, marker, haplogroup, state in parsed:
        if pos in recurrent_positions:
            continue
...
def add_to_final_table(
    ...
    # Valid_markers reports the QC-relevant count: exactly the markers that fed the
    # QC scores. haplotype_dict already excludes recurrent (homoplastic) positions
    # and ~ haplogroups, so this differs from the raw .info count when present.
    valid_markers = sum(linker.nr_total for linker in haplotype_dict.values())
...
def add_to_json_records(
    ...
    total_reads, valid_markers = process_info_file(folder / (folder.name + info_file_suffix))
    # QC-relevant count (see add_to_final_table): markers that fed the QC scores.
    valid_markers = sum(linker.nr_total for linker in haplotype_dict.values())
    hg, ancestral_children, qc1, qc2, qc3, total, _ = best_haplotype_scores
